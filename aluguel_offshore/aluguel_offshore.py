...
def puxa_ticker_client(x, pras_book, abertura_treas_ops):
    acc = abertura_treas_ops['conta_client'][x]
    shares = -abertura_treas_ops['qt_client'][x]
    ticker = abertura_treas_ops['ticker'][x]
    date = abertura_treas_ops['date'][x]
    
    try:
        sub_df =pras_book[(pras_book['PRODUCT_CODE.TICKER']==ticker)&\
                (pras_book['date']==date)&(pras_book['shares']==shares)]
        book = sub_df['Book'].reset_index(drop=True)[0]
    except IndexError:
        print ('%s -- %s -- %s -- %s' %(acc, shares, ticker, date))
        book = np.nan
    return book

client_acc_change_name_dict = {
    '2698':'BTG INT G BTG CAYMAN USD'
}
